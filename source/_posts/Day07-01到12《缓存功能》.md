...
flowchart LR
    subgraph 你现在要写的
        A[DishController<br/>用户端控制器]
    end
    
    subgraph 已经写好的-直接调用
        B[DishService<br/>业务接口]
        C[DishServiceImpl<br/>业务实现]
        D[DishVO<br/>数据对象]
    end
    
    subgraph 工具类-直接用
        E[RedisTemplate<br/>操作Redis]
    end
...
flowchart LR
    subgraph 菜品模块-手动方式
        A1[用户查菜品] --> A2[手动查Redis]
        A2 --> A3{命中?}
        A3 -->|是| A4[返回]
        A3 -->|否| A5[查DB → 手动存Redis]
        
        B1[管理员改菜品] --> B2[调用clearCache方法]
        B2 --> B3[redisTemplate.delete]
    end
    
    subgraph 套餐模块-SpringCache
        C1[用户查套餐] --> C2["@Cacheable<br/>一行搞定"]
        
        D1[管理员改套餐] --> D2["@CacheEvict<br/>一行搞定"]
    end
```
