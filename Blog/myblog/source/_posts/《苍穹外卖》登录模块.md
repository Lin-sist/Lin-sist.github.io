...
flowchart TD
    ...
    F --> G[后续请求带token]
    G --> H[Interceptor验JWT]
    H --> I[ThreadLocal保存用户ID]
    I --> J[业务Service读取当前用户ID]
```
...
sequenceDiagram
    ...
    rect rgb(255, 200, 200)
        Note over 前端,JWT工具: 【场景一：登录请求 - 不经过拦截器！】
        前端->>Controller: POST /admin/employee/login<br/>{username, password}
        Note over Controller: ⚠️ 登录接口在WebMvcConfiguration中<br/>被excludePathPatterns排除了<br/>所以不会被拦截器拦截！
        ...
        Service->>Service: 三道校验（存在？密码？锁定？）
        Service-->>Controller: 返回Employee
        Controller->>JWT工具: 生成Token
        JWT工具-->>Controller: 返回token字符串
        Controller-->>前端: {id, name, token} ✅ 登录成功
    end
    
    rect rgb(200, 255, 200)
        Note over 前端,JWT工具: 【场景二：访问其他接口 - 拦截器工作！】
        前端->>Controller: GET /admin/dish/page<br/>Header: token=xxx
        Note over Controller: 👮 拦截器拦截！
        Controller->>Controller: JwtTokenAdminInterceptor.preHandle()
        Controller->>JWT工具: 解析token
        JWT工具-->>Controller: 返回empId
        Controller->>Controller: BaseContext.setCurrentId(empId)
        Note over Controller: ✅ 放行
...
```mermaid
graph LR
    A[Service调用] -->|employeeMapper.getByUsername| B[Mapper接口]
    B --> C[MyBatis代理对象]
...
graph LR
    A[请求到达] --> B[preHandle]
    B -->|return true| C[Controller执行]
    C --> D[postHandle]
    D --> E[视图渲染]
    E --> F[afterCompletion]
    F --> G[响应返回]
    
    B -->|return false| H[直接返回]
...
sequenceDiagram
    ...
    rect rgb(255, 230, 230)
        Note over 前端,TL: 【场景一：登录】不经过拦截器
        前端->>Controller: POST /admin/employee/login<br/>{username:"admin", password:"123456"}
        Note over Controller: DTO接收参数，防止参数污染
        Controller->>Service: login(employeeLoginDTO)
        Service->>Mapper: getByUsername("admin")
        Note over Mapper: MyBatis动态代理
        Mapper->>DB: SELECT * FROM employee WHERE username='admin'
        DB-->>Mapper: 查询结果
        Mapper-->>Service: Employee对象
        Service->>Service: ① 账号存在？<br/>② MD5密码比对<br/>③ 账号未锁定？
        Service-->>Controller: 返回Employee实体
        Controller->>JWT: createJWT(secretKey, ttl, {empId:1})
        JWT-->>Controller: token字符串
        Note over Controller: VO返回数据，防止密码泄露
        Controller-->>前端: {id, userName, name, token}
    end
    
    rect rgb(230, 255, 230)
        Note over 前端,TL: 【场景二：访问资源】经过拦截器
        前端->>Interceptor: GET /admin/dish/page<br/>Header: token=xxx
        Interceptor->>Interceptor: ① 静态资源？直接放行
        Interceptor->>JWT: parseJWT(secretKey, token)
        JWT-->>Interceptor: Claims{empId:1, exp:...}
        Interceptor->>TL: setCurrentId(1)
        Note over TL: 存入当前线程的Map
        Interceptor->>Controller: 放行
        Controller->>Service: 处理业务
        Service->>TL: getCurrentId()
        TL-->>Service: empId=1
        Note over TL: 从当前线程的Map取出
        Service-->>Controller: 业务数据
        Controller-->>Interceptor: 返回响应
        Interceptor->>TL: removeCurrentId()
        Note over TL: ⚠️清理！防止内存泄漏
        Interceptor-->>前端: 响应结果
    end
```
