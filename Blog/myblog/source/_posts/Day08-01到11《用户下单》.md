...
flowchart TB
    A[用户点击提交订单] --> B{校验收货地址}
    B -->|地址为空| ERROR1[抛出异常:地址为空]
    B -->|地址存在| C{检查配送范围}
    
    C -->|超出范围| ERROR2[抛出异常:超出配送范围]
    C -->|范围内| D{校验购物车}
    
    D -->|购物车为空| ERROR3[抛出异常:购物车为空]
    D -->|有商品| E[构建订单数据]
    
    E --> F[生成订单号<br/>System.currentTimeMillis]
    F --> G[设置订单状态<br/>PENDING_PAYMENT]
    G --> H[插入orders表]
    H --> I[遍历购物车]
    I --> J[构建订单明细]
    J --> K[批量插入order_detail表]
    K --> L[清空购物车]
    L --> M[返回OrderSubmitVO]
